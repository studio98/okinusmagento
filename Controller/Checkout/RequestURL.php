...
class RequestURL extends \Magento\Framework\App\Action\Action{
    ...
    public function getItems($quote){
        ...
        foreach($quote->getTotals() as $total){
            $data = $total->getData();
            if(@$data['code'] == 'amasty_extrafee'){
                $extraFeeTotal = @$total['value'];
            }
        }

        // IF extra fee is a number and greater than 0
        if(isset($extraFeeTotal) && $extraFeeTotal > 0){
            $result[] = [
                'sku' => '0000',
                'quantity' => 1,
                'unit_price' => $extraFeeTotal,
                'description' => 'Shipping',
            ];
        }
